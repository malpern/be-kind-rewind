...
- `CreatorThemeClassifier` — batch Claude Haiku call that takes ~200 video titles for a creator and returns:
  1. **5-10 named theme clusters** with descriptions and member video IDs
  2. **Series detection** as a secondary output — for each cluster (or as separate cluster types), whether it looks like a sequenced series (member videos can be ordered) and the inferred ordering signal (numeric like `Day N` / `Episode N`, date-based, or unordered). The classifier handles both *enumerated* series ("Day 1-30") and *thematic* series ("I tried X for a week" pattern with different X each time). One LLM call does both jobs — there's no separate regex pre-pass.

  Decision rationale: an earlier draft of this plan had a regex pre-pass for enumerated series patterns as a free pre-filter. That was a premature cost optimization — once we've already accepted the LLM cost for theme clustering (~$0.001-0.005 per creator, cached), adding series detection to the same call is essentially free and catches both enumerated and thematic series in one pass with one code path. See Appendix E item 4 for the discussion.

- `creator_themes` SQLite table for caching results, including a per-cluster `is_series` flag and optional `ordering_signal` column.
...
- **Standout episodes within detected series.** Phase 2's `CreatorThemeClassifier` returns clusters with an `is_series` flag (covering both enumerated patterns like `Day 1-30` and thematic patterns like `I tried X for a week`). Phase 3 surfaces the **standout episode of each series** — the outlier within the series subset, computed via `OutlierAnalytics.topOutliers(...)` scoped to the series videos rather than the channel as a whole. Answers "what's the must-watch episode of this series?"

  Implementation is small (~30 lines on top of the existing `OutlierAnalytics` module): for each series cluster from the cache, take its member videos, run them through `topOutliers` to get the local-median outlier, and surface that one as the series' "standout." A series with no clear standout (all episodes within 2× of the series median) just shows no badge.

  Note: an earlier draft of this plan had a separate regex pre-pass for series detection in Phase 2. That was removed (see Phase 2 note above and Appendix E item 4) — series detection now happens in the same LLM call as theme classification, and Phase 3 just consumes its output.
...
### 6. Series detection — LLM only, no regex pre-pass

**Original plan:** Phase 2 had a regex pre-pass for enumerated patterns (`Day N`, `Episode N`, `Part N`) followed by an LLM call for thematic patterns. Two code paths, two cache layers, hybrid result merging.

**Reframed:** The LLM theme classifier already needs to run for tag capsules and the "By theme" section. Once we've accepted that LLM cost (~$0.001-0.005 per creator, cached forever), having it also detect series in the same call is essentially free. The LLM catches both enumerated patterns AND thematic patterns ("I tried X for a week" with different X each time) that hand-written regex would miss.

The regex pre-pass remains *technically* faster (microseconds vs. ~1 second + network round trip on first page open), but with caching the LLM only runs once per creator and the latency is invisible after that.

**Decision:** drop the regex pre-pass entirely. `CreatorThemeClassifier` returns clusters with an `is_series` flag and an optional `ordering_signal` (numeric / date-based / unordered). One code path, one cache table, simpler implementation.

**Phase 3 standout episodes feature is unchanged** — it consumes whatever series the Phase 2 classifier returns and applies series-scoped outlier scoring on top. The "find the breakout episode of a series" logic is independent of how the series was detected.

Conversation context: surfaced when user noted they were OK with cheap LLM costs, asked whether the regex pre-pass was still worth doing. Cost optimization that pre-dated the cheap-LLM acceptance is no longer needed.
