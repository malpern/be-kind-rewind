...
## Phase 2 — Themes, About, and similar creators (LLM-driven)

**Scope:** Add the LLM-powered enrichments. New persistence for caching results so we are not paying per page-open.
...
- Similar creators ("Creators you might like") computed from topic overlap, no LLM needed:
  ```
  overlap(A, B) = (# topics both publish in) / (# topics A publishes in)
  ```
  Returned sorted by overlap, top 8.
...
| 2 | LLM themes, about paragraph, similar creators | ~1 week | ~$0.005 / creator, cached | `creator_themes`, `creator_about` | LLM caching + cost discipline pattern |
